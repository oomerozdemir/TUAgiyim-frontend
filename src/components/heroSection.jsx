...
export default function EnhancedHeroSection() {
  ...
  // Renkler: Daha soft, beige ve stone tonları
  const features = [
  ...
  return (
    ...
    >
      ...
      {/* Floating Particles - Artık daha gri/bej */}
      ...
      {/* Gradient Orbs - Çok soft vizon renkleri */}
      <div 
        className="absolute top-20 right-20 w-96 h-96 bg-gradient-to-br from-[#D6C4A8]/10 to-[#A39075]/10 rounded-full blur-3xl animate-pulse"
        style={{
          transform: `translate(${(mousePos.x - 0.5) * -20}px, ${(mousePos.y - 0.5) * -20}px)`
        }}
      />
      <div 
        className="absolute bottom-20 left-20 w-[500px] h-[500px] bg-gradient-to-tr from-[#C5B398]/10 to-[#EBE5D9]/20 rounded-full blur-3xl"
        style={{
      ...
      <div className="max-w-[1600px] mx-auto px-6 w-full grid grid-cols-1 lg:grid-cols-12 gap-12 items-center relative z-10">
        ...
        <div className={`lg:col-span-5 flex flex-col justify-center text-center lg:text-left space-y-6 transition-all duration-1000 transform ${loaded ? "translate-y-0 opacity-100" : "translate-y-10 opacity-0"}`}>
          ...
          <div className="flex flex-col sm:flex-row items-center gap-4 justify-center lg:justify-start pt-4">
            ...
            <button
              className="group flex items-center gap-2 text-black/70 hover:text-[#8C7B62] font-medium text-sm relative pb-1 transition-colors"
              onClick={() => navigate("/hakkimizda")}
            >
              <span className="relative">
                Hikayemizi Oku
                <span className="absolute bottom-0 left-0 w-0 h-px bg-[#8C7B62] group-hover:w-full transition-all duration-300" />
              </span>
            </button>
          </div>
